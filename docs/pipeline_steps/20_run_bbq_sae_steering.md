...
**Status:** Closed for the feature-intervention paths (`clamp`, `steer`) in commit `22e8345` (`scripts/encode_identity_saes.py` primitives + `scripts/run_bbq_sae_steering.py` runner). Implements the audit's recommended option 1 below (per-feature `feature_stats.csv` lookup × multiplier grid); the RMS-norm alternative (option 2) was not implemented. `ablate` (the headline default) is unaffected.

**What landed:**
- `clamp_features` accepts a per-feature target tensor (aligned with `feature_ids`); `steer_features` gains a per-feature `scale`. New CLI `--feature_scale_stat {none,p95,p99,max,mean_nonzero}` (default `p95`), `--feature_stats_dir` (per-layer `layer_<NN>/feature_stats.csv`), `--clamp_multipliers` (grid, default `1.0`).
- Under a stat: **clamp target = `clamp_multiplier × scale[f]`**, **steer increment = `alpha × sign × scale[f]`**. So `α` (the multiplier for clamp, alpha for steer) means the same thing — "how many p95s of this feature's own activation" — for every feature. `--feature_scale_stat none` reproduces the pre-3.2 uniform path (clamp uses scalar `--clamp_value`; steer adds raw `alpha`).
- `clamp` now sweeps `--clamp_multipliers` as its grid when scaled (each multiplier a distinct job, recorded in the `alpha` column). Job-id / resume / progress counting are mode-aware (`alpha_grid_for_mode`, `jobs_per_example_for_modes`) so the multiplier grid is counted correctly.
- Two new output columns (none removed): `feature_scale_stat` and `feature_scale_value` (single-feature scale; NaN for bundles/ablate). Effective clamp target = `alpha × feature_scale_value`; effective steer increment = `alpha × sign × feature_scale_value`.

**Why it matters (now fixed):** cross-feature effect-size comparisons in Step 23's `feature_effect_rankings.csv` and `final_intervention_candidates_table.html`, when derived from a clamp/steer headline, are now on a common per-feature scale rather than a mixture of feature importance and feature-specific scale mismatch.

**⚠️ Pre-RunPod validation gap:** synthetic checks (30/30) confirm the per-feature math and the cross-feature comparability property on fabricated stats; they do **not** exercise the real triage feature pool against the real saved `feature_stats.csv`. Silent-failure modes against real data: (i) a kept feature absent from `feature_stats.csv` → scale 0 → clamp degenerates to ablate / steer to a no-op; (ii) a kept feature with non-positive `p95` (dead/rare, or `fillna(0)`) → same collapse; (iii) wrong `--feature_stats_dir` or an un-encoded layer → load-time error. **Use `scripts/audit_feature_scale.py`** before any clamp/steer headline; it reads the triage pool + per-layer `feature_stats.csv` and exits non-zero if too many kept features are degenerate. The `ablate` headline does not depend on any of this.

**Original audit (preserved):**

Even within the current direction-addition design, `--alphas=-8,-4,-2,2,4,8` is applied to a unit vector identically for every feature. Features differ in natural activation magnitude (some `feature_stats.p95` are 0.1, others 5+) and decoder norm. A constant `alpha` is neither "amplify feature `f` by X%" nor a fixed fraction of the residual-stream RMS norm at that layer/position (which is itself ~3–10× the unit vector). So one feature's "big effect at α=8" might just reflect a *relatively larger* perturbation than another's "small effect at α=8". The same problem applied to the `clamp`/`steer` codepaths via the single scalar `--clamp_value`. `ablate` is unaffected (it sets the latent to exactly 0).

Targeted fix options the audit listed: **(1, implemented)** per-feature clamp value from each feature's own `p95`/`p99`/`max` in `feature_stats.csv` × a multiplier grid, so `alpha` becomes "ablated" (0) or "amplified to k×p95"; **(2, not implemented)** scale `alpha` to a fixed fraction of the median residual-stream RMS norm at that layer/position.
...
**Status:** Closed in commit `9b0fac8` — **superseded by the 3.1 feature-intervention path**. A bundle feature set (`per_contrast_topk` / `role_bundle`, >1 `feature_id`) run under `ablate`/`clamp`/`steer` goes through `install_feature_intervention_hook` with the full `feature_ids` list and edits every latent in the set **simultaneously**: the bundle effect is the joint causal effect of the set, exactly the audit's targeted fix. Under the audit-3.1 default `--intervention_modes ablate` this is already what runs; under audit 3.2 a bundle `clamp`/`steer` scales each member by its own `feature_stats` value. The averaged `make_vector` direction is now reached only by the legacy `add_vector`/`ablate_projection` modes and the direction-shaped controls.

**What landed:** a soft guard in `main()` warns when bundle sets will run under a legacy averaged-decoder-vector mode (pointing to the set-intervention modes; legacy retained for the 5.5 baseline), and `make_vector`'s docstring is marked legacy/baseline-only. No behavioral change to the `ablate` default or the output schema (bundles were already tagged `feature_estimate_type="feature_bundle"`, `feature_id` sentinel `-1`; analyzer tags them `feature_bundle_membership`). Synthetic 13/13: bundle `ablate` zeros all members; bundle `clamp` sets each member to `mult × scale[f]` (three distinct targets, not one averaged value); dispatch needs no averaged vector; metadata tagged correctly.

**Original audit (preserved):** `per_contrast_topk` and `role_bundle` modes build `vec = unit-norm mean of signed unit-normed decoder rows`. The downstream analyzer correctly tags these `feature_bundle_membership` and warns. But averaging unit-normed rows and then re-normalizing produces a direction whose relation to any individual feature is weak — bundle effects are hard to interpret even as "membership." Targeted fix: once 3.1 lands, redefine bundle interventions as "clamp every latent in the set to 0 (or to p95)" — the bundle effect becomes the joint causal effect of the set. (Alternatively deprecate bundle modes and run `per_feature` only; the production command already passes `--require_per_feature`.)
