...
| 07 | `analyze_identity_geometry.py` | 2 | **B** (2.2 — PROBE NULL LANDED) · M (2.1 — FIX LANDED, 4.1 — FIX LANDED) · m (2.8 — VERIFIER LANDED, 5.9 — FIX LANDED, 5.10 — FIX LANDED) · enabler (5.5 — DoM + probe directions persisted) | [07_analyze_identity_geometry.md](07_analyze_identity_geometry.md) |
...
| 12 | `plot_identity_directional_followups.py` | 2 | m (2.1 visualization — FIX LANDED, 4.1 — FIX LANDED, 5.10 — FIX LANDED) | [12_plot_identity_directional_followups.md](12_plot_identity_directional_followups.md) |
| 13 | `analyze_identity_sae_features.py` | 3 | M (2.1 — FIX LANDED, 2.5 — FIX LANDED, 4.1 — FIX LANDED, 5.1 — FIX LANDED) · m (5.3 — FIX LANDED, 5.4 — FIX LANDED, 5.10 — FIX LANDED) · enabler (3.1 — FIX LANDED) | [13_analyze_identity_sae_features.md](13_analyze_identity_sae_features.md) |
| 14 | `extract_token_level_sae_activations.py` | 3 | m (4.6 — PARTIAL FIX LANDED, 5.10 — FIX LANDED) · enabler (1.1) | [14_extract_token_level_sae_activations.md](14_extract_token_level_sae_activations.md) |
| 15 | `build_sae_feature_cards.py` | 3 | m (logit-lens caveat — FIX LANDED, 5.10 — FIX LANDED) | [15_build_sae_feature_cards.md](15_build_sae_feature_cards.md) |
...
| 20 | `run_bbq_sae_steering.py` | 4 | **B** (2.3 — FIX LANDED, 3.1 — FIX LANDED) · M (1.3 — FIX LANDED, 2.4 — FIX LANDED, 3.2 — FIX LANDED, 3.3 — FIX LANDED, 3.4 — FIX LANDED, 5.5 — PARTIAL FIX LANDED) · m (3.5 — FIX LANDED) | [20_run_bbq_sae_steering.md](20_run_bbq_sae_steering.md) |
...
| 23 | `analyze_bbq_feature_level_causal_effects.py` | 4 | **B** (4.3 — FIX LANDED) · M (2.4 — FIX LANDED, 2.5 — FIX LANDED, 2.6 — FIX LANDED) · m (2.7 — FIX LANDED) | [23_analyze_bbq_feature_level_causal_effects.md](23_analyze_bbq_feature_level_causal_effects.md) |
...
3. **Re-enable steering controls** (2.3 — FIX LANDED 2026-05-28, commit `42b5837`). Controls now run by default in BOTH scoring paths (slow per-example AND fast batched first-token). New `random_feature_ablate` control for the audit-3.1 ablate headline. `--controls_subsample_frac` cost knob (production: `0.20`). `--disable_controls` is now smoke-test-only. DoM + probe direction baselines already wired in via 5.5.
4. **Polarity-sign the bias metric** (4.3 — FIX LANDED 2026-05-27, commit `a03760f`). [Step 23](23_analyze_bbq_feature_level_causal_effects.md) now computes `signed_stereotype_preference_delta` in `enrich_results` and routes `effect_label`, `beneficial_score`, `harmful_score`, `make_rankings`, and every plot through the signed metric. The unsigned `mean_stereotype_preference_delta` is preserved alongside as a diagnostic.
...
- ~~**Step 20** — `--scoring_mode first_token --disable_controls` takes a separate batched code path with **no control code at all**; even with controls re-enabled (2.3 fix), the user must also drop `first_token` mode to get them.~~ ✅ Closed 2026-05-28 in commit `42b5837` (audit 2.3): controls are plumbed into the batched first-token path; `--disable_controls` is smoke-test-only.
