...
| 07 | `analyze_identity_geometry.py` | 2 | **B** (2.2 — PROBE NULL LANDED) · M (2.1 — FIX LANDED, 4.1 — FIX LANDED) · m (2.8 — VERIFIER LANDED, 5.9 — FIX LANDED, 5.10 — FIX LANDED) · enabler (5.5 — contrast directions persisted) | [07_analyze_identity_geometry.md](07_analyze_identity_geometry.md) |
...
3. **Re-enable steering controls** (2.3) — Step [20](20_run_bbq_sae_steering.md) ships with `sign_flip`, `random_direction_norm_matched`, `random_feature_matched`, but the production command uses `--disable_controls`. The diff-of-means baseline part is now done as a separate mode (5.5 — PARTIAL FIX LANDED 2026-05-27, commits `8f84e5e` + `a11cbb8`: new `direction_baseline` intervention mode + `--direction_baselines_path`; RunPod head-to-head pending). 2.3 controls themselves still open.
