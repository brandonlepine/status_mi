...
| 20 | `run_bbq_sae_steering.py` | 4 | **B** (2.3, 3.1 — FIX LANDED) · M (1.3, 2.4, 3.2, 3.3 — FIX LANDED, 3.4 — FIX LANDED, 5.5 — PARTIAL FIX LANDED) · m (3.5) | [20_run_bbq_sae_steering.md](20_run_bbq_sae_steering.md) |
...
3. **Re-enable steering controls** (2.3) — Step [20](20_run_bbq_sae_steering.md) ships with `sign_flip`, `random_direction_norm_matched`, `random_feature_matched`, but the production command uses `--disable_controls`. Both the DoM and logistic-probe baselines are now done as separate modes (5.5 — PARTIAL FIX LANDED 2026-05-27/28, commits `8f84e5e` + `a11cbb8` + `7cdb164` + `8c392d7`: `direction_baseline` + `probe_baseline` modes + their `--*-path` flags; RunPod three-way head-to-head pending). 2.3 controls themselves still open.
