...
> **Upstream callout — issue 1.4 (CONFIRMED).** This script joins per-feature metrics from `feature_selectivity_alignment_joined.csv`, `intervention_candidate_features.csv`, `feature_identity_selectivity.csv`, and the token-level activations. All of those derive from the broken encoder in [Step 5](05_encode_identity_saes.md#14-blocker--sae-preprocessing-convention-confirmed-wrong-concrete-fix-below). After the Step 5 fix, `feature_triage.csv` and `intervention_candidate_features_triaged.csv` must be regenerated — the role assignments and `keep_for_intervention` flags currently in those files were computed on wrong activations, so the feature pool that drove every BBQ steering result is itself wrong. The triage *logic* (issue 5.2) is independent.
